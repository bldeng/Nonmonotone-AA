...
class BaseTest(TestCase):
    ...
    def compare_total_all(self, results, names, savefig = None):
        ...
        for i in range(len(names)):
            ...
            plt.semilogy(range(result["num_iters"]), result["r_dr"],
                          label="Residuals (" + name + ")")
        plt.xlabel('Iterations')
        plt.ylabel('DR residual')
        plt.savefig('DRr_iteration.pdf', bbox_inches="tight")
        plt.legend()
        if savefig:
            plt.savefig(savefig, bbox_inches="tight")
        plt.show()
        ...
        for i in range(len(names)):
            ...
            # plt.semilogy(range(result["num_iters"]), np.sqrt(result["primal"]**2+result["dual"]**2),
            #              label="Residuals (" + name + ")")
            plt.semilogy(result["time_iter"], result["r_dr"],
                         label="Residuals (" + name + ")")
            # plt.title("Residuals")
        plt.title("Time")
        plt.xlabel('Time(s)')
        ...
        plt.ylabel('DR residual')
        plt.legend()
        plt.savefig('DRr_time.pdf', bbox_inches="tight")
        if savefig:
            plt.savefig(savefig, bbox_inches="tight")
        plt.show()
        plt.figure()
        ...
        plt.savefig('Totalr_iteration.pdf', bbox_inches="tight")
        if savefig:
            plt.savefig(savefig, bbox_inches="tight")
        plt.show()
        plt.figure()
        for i in range(len(names)):
            result = results[i]
            name = names[i]
            plt.semilogy(result["time_iter"], np.sqrt(result["primal"]**2+result["dual"]**2),
                          label="Residuals (" + name + ")")
            #plt.semilogy(range(result["num_iters"]), result["r_dr"],
            #             label="Residuals (" + name + ")")
            # plt.title("Residuals")
        ...
        plt.xlim(0,10)
        plt.ylabel('Total residual')
        plt.savefig('Totalr_time.pdf', bbox_inches="tight")
        if savefig:
            plt.savefig(savefig, bbox_inches="tight")
        plt.show()
